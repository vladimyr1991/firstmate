...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, endpoint_task_id=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=, backend=; kind=secondmate also records home= and projects=; herdr, zellij, orca, and cmux each record further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-pr-check, including through fm-pr-merge, records one canonical pr= and the forge's pr_head= when available (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for an X-mode-originated task (section 14); optional base= is recorded by hand rather than by fm-spawn and is read by the lessons-learned retro pass to count commits against the project's real ship base
  ...
  <id>.ci-run-poll   private validated forge/repo/run-id sidecar for a raw CI run watch (bin/fm-ci-run-check.sh), independent of any pull request; armed as an intentional custom check, not a byte-static template; the shared <id>.check.sh slot is never silently stolen: fm-ci-run-check.sh refuses loudly (exit 3) when a live PR merge poll owns it, and fm-pr-check.sh refuses the same way over a live CI run poll
  <id>.ci-run-poll-retirement  private one-shot receipt claimed best-effort by the generated check itself right after it prints a terminal result and before it retires every artifact, so printing never has a preceding side effect that could silently lose the result; a leftover receipt means an earlier firing was interrupted before finishing removal, recovered without a duplicate report by the next check execution or the next fm-ci-run-check.sh registration for the same task id (an interruption before the claim itself may instead cause one accepted duplicate report)
  .pr-check-quarantine/  private non-runnable storage for checks neutralized by the non-executing migration
  .pr-check-migration.log  private per-task outcomes distinguishing rebuilt or canonically registered replacement polls, quarantined unarmed polls, and incomplete migrations
  .pr-check-migration-scan-v1  private marker proving the non-executing scan disabled every unsafe legacy check; .pr-check-migration-v1 separately records completed private repairs
  x-watch.check.sh   generated X-mode relay poll shim; present only when opted in (section 14)
  quota-frozen/      durable obligations for work stopped by an exhausted model window, one private record per frozen task or role plus private `.notified/` wake markers; written only by bin/fm-quota-freeze.sh and removed only after a confirmed resume, so the obligation outlives the conversation and the wake it produced (section 13's quota-autoresume; bin/fm-quota-freeze-lib.sh)
  ...
  .gate-lock         full-test-gate queue hold; workers take and release it themselves through `bin/fm-gate.sh` and firstmate is never in that loop
...
- `captain-operations` - load at session start, before changing anything tracked, and whenever a setting's meaning or location is in doubt; carries the fleet's operating posture, the delivery flow, the session-start watcher duty, and the live configuration map.
