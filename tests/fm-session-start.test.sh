...
EOF
  ...
  assert_contains "$out" "is on feature branch 'fm/read-only-tangle'" "read-only bootstrap did not surface the tangle diagnostic"
  assert_contains "$out" "TANGLE: $root is on feature branch" "read-only tangle diagnostic did not name the stranded checkout"
