...
unit_clear_stale() {
  ...
  : > "$st/state/.subsuper-digest-1-2-3.txt"
  ...
  if [ ! -e "$st/state/.subsuper-escalations" ] \
     ...
     && [ ! -e "$st/state/.subsuper-digest-1-2-3.txt" ]; then
    pass "clear-stale: removes escalations buffer, sidecar, wedge marker, and digest files"
  else
