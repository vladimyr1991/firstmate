...
- **Optional voice input** - opt in with one local `config/voice` file to hold a key chord, speak, and have a local whisper.cpp transcription typed into the focused Herdr agent pane for you to read and send; nothing is recorded outside the hold, no audio leaves the machine, and a home without the file is unchanged.
...
- **Restart-proof** - all state lives on disk and in the active session backend (tmux, herdr, cmux, zellij, or orca; [`docs/configuration.md`](docs/configuration.md#runtime-backend-configbackend--fm_backend) owns how one is selected and which is the default); kill the session anytime and the next one reconciles, including confirmed-dead secondmate agents, and carries on.
