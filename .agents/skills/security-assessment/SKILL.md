...
Every finding carries four elements, and one missing any of them is incomplete:
...
Element 4 is the one most often dropped.
...
Where it cannot be built, the route table's own treatment above applies here unchanged: expect that refusal, satisfy the validation the same way, take the source search below as the enumeration itself rather than as a reconcile against a stack you do not have, and record in the finding that the enumeration was source-side and therefore a floor.
...
A form can go missing from that list without anything signalling it, because a layer installed by a form the built stack still records is found by the primary enumeration while only its attribution in the source reconcile fails, which is why the list has to be maintained even while the primary enumeration looks complete.
So attribute each layer the built stack shows to the code that installed it, and where the source installs a layer the built stack does not show, that layer has a reason and establishing it is the step, because the reason decides what is recorded: the enumeration may extend to a layer no built stack records, or the layer may be a fact about the environment being graded under the qualification the route table above already carries, or another object may serve paths and have to be enumerated too.
Those are instances rather than the set, and a reason outside them is possible and is established the same way, by reading which application object the layer is installed on and under what condition.
An application composed of more than one object needs one enumeration per object, each authoritative for the paths it serves, so record for each enumerated object which paths it governs and treat the account as a floor until every object that serves paths has been enumerated.
