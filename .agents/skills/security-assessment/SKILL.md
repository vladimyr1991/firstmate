...
That search is a reconcile for the forms the built stack records and the primary enumeration for the wrap form, since a layer registered on the application object lands in that object's own stack and is already shown there, while a wrap performed in application code builds a new outer object whose layer the inner object's stack never records.
So attribute each layer the built stack shows to the code that installed it, and read anything the source installs that the built stack does not show as a layer that stack was never going to record rather than as a discrepancy between the two readings, of which a wrap is the clearest instance and not the only one.
