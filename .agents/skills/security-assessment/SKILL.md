...
So attribute each layer the built stack shows to the code that installed it, and where the source installs a layer the built stack does not show, that layer has a reason and establishing it is the step, because the reason decides whether the enumeration extends to a layer no built stack records, whether it is a fact about the environment being graded under the qualification the route table above already carries, or whether another object serves paths and must be enumerated too.
