...
Then establish the following about the limiter itself, where what it keys on and whether it returns early outside prod are read in its body, while whether a client controls the keyed value and whether the counter is shared each need the deployment configuration as well and are not settled by the body alone.
