...
| Secrets | Credential exposure through logs, images, and CI | Find every place a credential is named by searching for the credential environment names themselves, `PLATFORM_AUTH_SECRET`, `PLATFORM_OPERATOR_TOKEN`, `PLATFORM_WORKER_TOKEN`, `PLATFORM_ENCRYPTION_KEY`, `TELEGRAM_BOT_TOKEN` and `GOOGLE_OAUTH_CLIENT_SECRET`, and find where a credential carried in a request URL would otherwise reach the logs by searching for the `create_app` factory and for the `httpx` and `httpcore` logger names it quiets. Check where each is read and where it could reach build output, CI logs or a request URL, and name any secret you find by its location and never by its value |
...
Then read the limiter itself and ask which value it keys a client on and which element of that value it takes where the value is a multi-valued header such as `X-Forwarded-For`, whether that value is one the client can set, whether its counters are shared across processes or held per process, and whether it returns early outside prod.
The limiter's own body settles two of those questions outright, which value it keys a client on together with which element of it the limiter takes and whether it returns early outside prod, and it settles the counter question only as far as the mechanism, whether the counts live in process-local state or in a shared store, none of which needs traffic to see.
Where the keyed value is a header the reverse proxy in front of the application handles, whether the client can set it is decided across the limiter and that proxy, so read the ingress half first, because two of its three outcomes settle the question on their own.
Where the limiter keys on something no proxy directive touches, such as the socket peer address or a credential the client presents, the ingress half decides nothing and the limiter's body settles the question alone.
The ingress half is whether the reverse proxy overwrites that header, appends the address it observed to whatever arrived, or passes it through untouched, which you establish by reading the reverse proxy's own configuration file, found under the deployment directory beside the compose files rather than in the application package, and where that configuration names no directive for the header the answer is the documented default of the directive actually in use rather than pass-through by assumption.
Before letting an overwrite settle it, establish that the proxy is the only path to the application, which you read from the deployment configuration in the ports it publishes and in whether the application's own port is reachable other than through the proxy.
Where that port is reachable other than through the proxy, the value is client-settable and no reading of the limiter changes that, because a client connecting straight to the application authors the whole list itself and so controls every element of it.
Where the proxy is the only path, a proxy that overwrites the header makes the value not client-settable whatever the limiter reads, and one that passes it through untouched makes it client-settable whatever the limiter reads, so in either of those you can stop there.
Only the appending case, still under a proxy that is the only path, leaves the answer open, and there the source half decides it, because a forwarded-for header is a comma-separated list: a limiter that takes the first element reads the end nearest the client and so still takes the value the client sent, while one that takes the element the proxy appended does not.
Whether a process-local counter changes what the limit bounds splits the same way, across the limiter and the deployment configuration, because process-local counts multiply the limit only where more than one process or replica serves that surface.
How many processes serve the surface is settled by the deployment configuration, in the replica or process count of the service definition and in the worker count the server command passes, so read that before grading the mechanism.
A single process serving the surface leaves the bound exactly where a shared store would put it, and only a multi-process deployment multiplies it by the number of processes.
Both of those reads, the reverse proxy configuration and the deployment configuration, are configuration reads rather than active checks, so both stay inside the boundary above and neither needs traffic.
Driving traffic past the limit is resource-exhaustion testing and is forbidden by the active-check boundary above, and a property that neither read above can reach falls under the rule above on what stays unproven.
