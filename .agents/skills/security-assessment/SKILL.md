...
The limiter's own body settles three of those four: which value it keys a client on, whether its counters are process-local state rather than a shared store, and whether it returns early outside prod, and none of the three needs traffic to see.
Whether the keyed value is one the client can set is settled instead at the ingress in front of the application, by reading how the reverse proxy configuration treats that header before the request reaches the limiter, since a proxy may overwrite it, append to it, or pass it through untouched and the limiter's body shows none of that.
That is a configuration read rather than an active check, so it stays inside the boundary above and needs no traffic either.
...
- When a check in the third column returns nothing at the ref you are reviewing, establish whether the surface moved or went away, and route a correction in the same pass for whichever it was: the corrected search key where the surface moved, or the removal of that check, or of the row when nothing it names still exists, where the surface is gone.
