...
Before recording either branch, search the second way the answer could be expressed - the route strings, path prefixes and callers the key sat among - and where that finds the surface under a changed key, the correction is the new key.
Where the second search is also empty, record that the surface's existence is not established at your ref rather than that it is gone, because one empty grep does not establish absence over a tree.
...
Only when both searches come back empty may the finding say the guard appears unpinned at your ref, and it must say "appears" and name both searches that returned nothing, because a negative over a whole suite is not something grep can prove; either way, what the suite pins is context for composing element 4 and tells you where the probe you compose belongs.
...
Enumerate every router registered in the application factory, every route decorator under the API package, and every route declared directly on the application object inside the factory itself, then classify each by whether the route or its router carries an auth dependency from the `require_*`, `resolve_operator` and `scoped_tenant` family or a route-specific guard.
  A route declared on the app object sits in neither of the first two sweeps, which is exactly how a public endpoint hides from an enumeration that only walks routers and the API package.
...
The limiter's own body settles three of those questions on its own: which value it keys a client on together with which element of it the limiter takes, whether its counters are process-local state rather than a shared store, and whether it returns early outside prod, and none of the three needs traffic to see.
Whether that keyed value is one the client can set is settled by neither place alone, and both halves have to be read before the question is answered at all.
The source half is which element the limiter extracts, because a forwarded-for header is a comma-separated list: taking the first element reads the end nearest the client, while taking the last element, or reading a header the proxy sets outright, reads a different end.
...
Only the two together answer it, and the combination that most often turns a configuration assumed safe into a real bypass is a proxy that merely appends paired with a limiter that takes the first element, since the value the client sent is then still sitting in the position the limiter reads.
