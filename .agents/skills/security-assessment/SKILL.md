...
Then search the source for the framework's registration call such as `add_middleware`, for its decorator form such as `@app.middleware`, and for the assignment shape a wrap takes, where the name the served application object is bound to, such as the `app` the server invocation's application path string names, is passed into a constructor and bound back to a name.
...
It may be a layer registered on an application object other than the one whose stack was built, a mounted sub-application's own object or a second factory, which means the enumeration was run against the wrong object and is run again against that one.
