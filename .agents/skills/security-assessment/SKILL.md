...
| Prompt assembly and guardrails | Prompt injection; system prompt leakage | `voice_ai_workers/prompt_spec.py`, `voice_ai_workers/prompts.py`, `voice_ai_workers/guardrails/checker.py`, the `guardrails` and `out_of_scope` fields in `voice_shared/spec.py` |
| Public landing and pilot form | Unauthenticated write surface; anonymous free text stored and relayed verbatim into an operator-facing channel; unvalidated personal data taken on an unauthenticated write path; abuse of a billed resource | `voice_platform/api/pilot_requests.py`, `voice_platform/api/tokens.py`, `voice_platform/ratelimit.py`, `voice_platform/notify.py` (`notify_pilot_request`, and the `name`, `contact` and `note` it interpolates). Markup injection into Telegram is not the class here: `parse_mode` appears nowhere in `voice_platform`, and `voice_platform/telegram_api.py` posts a plain JSON payload, so the message renders as plain text |
| Operator and client screens | Token handling in the browser, including a JWT placed in a URL query string where proxy logs, browser history and referrer headers can see it; authorization enforced only in the UI | `frontend/src/auth.jsx` (the token in `localStorage` under `vp_token`), `frontend/src/api.js` (the `Authorization` header it attaches), `frontend/src/screens/CallArchive.jsx` and `frontend/src/screens/PortalCall.jsx` (each builds an audio `src` carrying `?token=`), `frontend/src/App.jsx` (the `role` route guards) |
| Telephony and channels | Provisioning against an external provider; admission and plan enforcement on the worker-authenticated internal path, where the caller's identity arrives from the telephony dispatch rather than from a request the platform authenticates | `voice_platform/api/sip.py` (mounted at `/api/internal/sip` behind `require_worker`, mints the `Call` row and runs `quota.check_quota`), `voice_platform/api/channels.py`, `voice_platform/sip_providers.py`, `voice_platform/sip_provision.py` |
...
| Public cached proxies | Unauthenticated outbound amplification and unbounded consumption, in that an anonymous caller with no credential makes the server issue an outbound call on its behalf on a cache miss; dependence on a third party the specialist may not probe. Not forgery: neither destination is caller-controlled, so do not label these server-side request forgery without code that lets the request choose where the server goes | `voice_platform/api/worker_status.py` (`_probe`, `_CACHE_TTL_S` of about two seconds, `worker_health_url` in `voice_platform/settings.py`), `voice_platform/api/fx.py` (`_fetch`, `_CBR_URL` at the third-party `cbr-xml-daily.ru`, `_TTL_S` of about twelve hours, `_FALLBACK`), and in both the point where `_cache["at"]` is re-stamped relative to the await |
...
That class attaches on the day a booking tool appears among the `function_tool` registrations in `voice_ai_workers/agent.py`, and not before.
Do not wait for a worker module to import `voice_platform.scheduling`, because that import is designed never to happen: the worker branches on the booking vocabulary through `voice_shared/booking_errors.py`, which imports the standard library and nothing else with a test enforcing that by AST, so the second observable to watch is `voice_ai_workers/test_booking_error_tool_contract.py` losing its opening premise that no booking tool exists yet.
...
The sixth carries two grades at once, and a finding there has to say which of the two halves it sits in.
`GET /api/integrations/google-calendar/callback` in `voice_platform/api/integrations.py`, mounted from `callback_router`, carries no `Authorization` header, and its entire authentication is the `oauth_state_claims` dependency, which runs `decode_oauth_state` over a `state` query parameter the caller supplies in full.
That validation path is anonymous, and `voice_platform/auth.py` says so itself, that a forged state is an ordinary thing to receive on a public redirect target, so a defect in the signature check, the `typ` check, or the freshness window is reachable by someone who has never held a real state.
Everything the route does after a valid state, the code exchange and the connection write and the redirect, is reachable only by whoever can obtain one, and `create_oauth_state` mints those inside `POST /connect` behind `scoped_tenant` and `resolve_operator`.
