...
A token named as the object of an existence check is the second kind: it names a conventional location or filename whose presence or absence is itself the answer being sought, so it need not resolve, and each row must make plain which kind it names so that an empty result is never read as a broken pointer.
...
That all-forms-empty result owes a terminal step rather than nothing, since the check would otherwise rot with no signal: establish from evidence outside the key list whether the thing is genuinely installed in no form or is installed in one the list does not name, and where it is the second, route the missing form as a correction the way a stale locator key is routed.
...
Nor does it govern a key naming alternative forms, where an empty result for one form owes no second search and is read as the rule for that kind above states, while an all-forms-empty result owes that rule's terminal step instead.
...
That completeness is also only over paths that reach routing at all, since middleware runs ahead of routing and can answer a request itself, so a path it handles is served by the application while appearing in no route table: the enumerations that must all be read are what routing serves, what answers before routing, and what the server or the deployment applies outside the application object, no one of them alone is the set of paths the application serves, and together they remain a floor rather than that set.
...
Enumerate the application-level middleware from the built application object's own stack, what the application actually has installed once the factory has constructed it, which the framework exposes on that object as it exposes the route table, and under the qualifications the route table above already carries, since the stack is the one the settings in force when the object was built produce and the same startup validation can refuse to build it.
Where it cannot be built, the route table's own treatment above applies here unchanged: expect that refusal, satisfy the validation the same way, take the source search below as the enumeration for every form rather than as a reconcile for some of them, and record in the finding that the enumeration was source-side and therefore a floor.
Then search the source for the framework's registration call such as `add_middleware`, for its decorator form such as `@app.middleware`, for the framework's own construction parameter such as the `middleware` argument passed where the application object is built, and for the assignment shape a wrap takes, where the name the served application object is bound to, such as the `app` the server invocation's application path string names, is passed into a constructor and bound back to a name.
Those are named here as alternative forms of one installation rather than as locators, and the wrap form is keyed on that assignment shape rather than on any particular type because the wrapper's own class name is not knowable in advance.
A form can go missing from that list without anything signalling it, because a layer installed by a form the built stack still records is found by the primary enumeration and only its attribution here fails, which is why this list has to be maintained even while the primary enumeration looks complete.
That search is a reconcile for the forms the built stack records and the primary enumeration for the wrap form, since a registration on the application object that no environment condition guards lands in that object's own stack and is already shown there, while a wrap performed in application code builds a new outer object whose layer the inner object's stack never records.
So attribute each layer the built stack shows to the code that installed it, and where the source installs a layer the built stack does not show, establish the reason before recording anything, since the reason decides what is recorded.
It may be a layer no built stack records because it wraps the application object or is applied outside it, of which a wrap is the clearest instance, and that extends the enumeration rather than marking a discrepancy between the built stack and the source.
It may be a layer this environment does not register while another does, which is no gap in the enumeration at all but a fact about the environment being graded, under the environment qualification the route table above already carries.
It may be a layer registered on an application object other than the one whose stack was built, a mounted sub-application's own object or a second factory, which does not make the object you enumerated the wrong one.
A parent's stack governs the paths the parent serves, including a mount's, while the mounted object's stack governs the paths inside that mount, so each is authoritative for its own paths and the step is to enumerate the other object as well rather than to replace what you already have.
Record for each enumerated object which paths it governs, since an application composed of more than one object needs one enumeration per object, and the account stays a floor until every object that serves paths has been enumerated.
A reason outside those is possible and is established the same way, by reading which application object the layer is installed on and under what condition.
The deployment chooses which entry point runs and what wraps the application before it is served, so a layer can be installed by the server invocation in an application module the built-stack enumeration never constructs, or outside application source entirely by a proxy, a server or a sidecar the deployment configures.
Both are found by reading how the application is actually invoked and deployed rather than by reading the factory, which is why this is its own place to look, and the enumeration is a floor until that read is done.
...
Grade a path that middleware answers with the same classification questions asked of a route-table entry above, put to the middleware that answers it rather than to a route's dependencies, so the enumerations converge on one classified set rather than a graded list beside a loose one.
...
- When every form a key naming alternative forms names returns nothing at the ref you are reviewing, take the terminal step that kind's rule above requires before concluding anything, and route in the same pass whichever it establishes: the form actually in use as a corrected key, or the record that the thing is installed in no form the key names and that whether those are still the right forms is not established.
...
- The terminal step drawn above establishes that a thing whose every named form returned nothing is installed in a form that key does not name.
