...
| Secrets | Credential exposure through logs, images, and CI | Regenerate the set of credentials rather than recalling it, by enumerating the credential declarations in the example environment file at the repository root, which is the manifest, together with the settings fields that read them, and then search for each name you get that way; `PLATFORM_AUTH_SECRET` and `TELEGRAM_BOT_TOKEN` illustrate the shape those names take and are not the set. Find where a credential carried in a request URL would otherwise reach the logs by searching for the `create_app` factory and for the `httpx` and `httpcore` logger names it quiets. Check where each is read and where it could reach build output, CI logs or a request URL, and name any secret you find by its location and never by its value |
...
The limiter's own body settles on its own which value it keys a client on together with which element of it the limiter takes, and whether it returns early outside prod, and it settles the counter question only as far as the mechanism, whether the counts live in process-local state or in a shared store, none of which needs traffic to see.
Where the keyed value is a header the reverse proxy in front of the application handles, whether the client can set it is decided across the limiter and that proxy, and the branches below settle it in the order they are given.
...
Establish first whether the application's own listener can be reached from outside the proxy at all, which you settle from the address that listener is bound to together with whatever network or firewall policy sits in front of it, and treat a port appearing under a publish directive as a hint rather than an answer, since a mapping bound to a loopback or private address, or a port a firewall closes, is published and still unreachable.
Where the application can be reached other than through the proxy, the value is client-settable whatever the limiter reads, because a client connecting straight to it authors the whole list itself and so controls every element of it.
Otherwise, where the proxy passes the header through untouched, the value is client-settable whatever the limiter reads.
Otherwise, where the proxy overwrites the header, the value is not client-settable whatever the limiter reads.
Otherwise the proxy appends, and that is the branch where the element the limiter reads decides, because a forwarded-for header is a comma-separated list: a limiter that takes the first element reads the end nearest the client and so still takes the value the client sent, while one that takes the element the proxy appended does not.
