...
Then search the source for the framework's registration call such as `add_middleware`, for its decorator form such as `@app.middleware`, and for the assignment where one application object is wrapped by another, which are named here as alternative forms of one installation rather than as locators.
...
So attribute each layer the built stack shows to the code that installed it, and establish which of two readings a layer the source installs but the built stack does not show has before recording either.
It is a layer no built stack records because it wraps the application object or is applied outside it, of which a wrap is the clearest instance and not the only one, and that extends the enumeration rather than marking a discrepancy between the two readings.
Or it is a layer this environment does not register while another does, which is no gap in the enumeration at all but a fact about the environment being graded, under the environment qualification the route table above already carries.
