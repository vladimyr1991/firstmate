...
Then reconcile that stack against the source by searching for the framework's registration call such as `add_middleware`, for its decorator form such as `@app.middleware`, and for the assignment where one application object is wrapped by another, which are named here as alternative forms of one installation rather than as locators, attributing each installed layer to the code that installed it and catching anything the source installs that the built stack does not show.
A layer applied outside the application object, by the server that serves it or by the deployment that wraps it, appears in neither the built stack nor those source forms, so read the server invocation and the deployment configuration as the third place a layer is applied, and treat the enumeration as a floor until that read is done.
