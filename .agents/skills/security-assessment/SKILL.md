...
So attribute each layer the built stack shows to the code that installed it, and read a wrap that only the source search finds as a layer the built stack was never going to show rather than as a discrepancy between the built stack and the source.
A layer applied by the server invocation that serves the application or by deployment configuration, rather than by application code, is written in no application source at all and wraps the application object from outside, so it appears in neither the built stack nor that search.
Read the server invocation and the deployment configuration as a further place a layer is applied, and treat the enumeration as a floor until that read is done.
