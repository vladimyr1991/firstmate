...
Take the axis to be anonymity rather than the absence of an `Authorization` header, because those two axes disagree and collapsing them is what makes this paragraph go wrong.
The set below is closed at the ref named at the head of the map, established by enumerating every router registered in `voice_platform/main.py`, which adds `CORSMiddleware` and no app-level auth dependency of its own.

`rate_limit` has exactly two call sites, and both are on this list: the login route at `voice_platform/api/auth_routes.py` line 39, and the visitor token mint at `voice_platform/api/tokens.py` line 105.
The other four are mounted in prod, are reached with no login at all, and are bounded by nothing the limiter knows about:
...
- `GET /api/integrations/google-calendar/callback` in `voice_platform/api/integrations.py`, mounted from `callback_router`, whose only authentication is the signature and freshness of the `state` minted at connect time.

Three groups sit beside that set and must not be folded into it, because each is a different question.
