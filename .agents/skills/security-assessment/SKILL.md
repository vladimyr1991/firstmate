...
A key must name a token that is present at the ref, so a token you established to be absent is a check result and can never become a key.
...
Both of those outcomes rest on the proxy being the only path to the application, which you establish from the deployment configuration by reading which ports it publishes and whether the application's own port is reachable other than through the proxy.
Where that port is reachable directly, a client can set the header itself without ever traversing the proxy, so no directive of the proxy's settles anything and the source half becomes decisive again.
