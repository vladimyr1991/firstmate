...
The printed `baseline_pointers=` / `baseline_boundaries=` counts (formerly `pointers=` / `boundaries=`) describe the baseline's counted set, not the working tree; `boundaries_now=` on the same per-skill line counts the working-tree statements the guard inspected, and that line is not printed at all for an unchanged file, while the summary line's `inspected_boundaries=` and `uninspected_skills=` report the aperture on every successful run, and none of those fields is coverage of a skill's content.
Do not cite `bin/fm-skill-compact-check.sh` as coverage of a specific file, or as the reason to add no other check, while those counts are zero, `boundaries_now=` or `inspected_boundaries=` is zero, or the skill is named `NOT COVERAGE`.
When the file has not changed yet and the per-skill counts are therefore not printed, the probe is to delete one line the change claims the guard protects, re-run, and restore; exit 0 on that probe is the same fact as zeros.
...
- Never let a `{ ... } > file` group carry a write whose failure matters: bash 3.2, the stock macOS `/bin/bash`, does not treat a failed redirection of a compound command as a `set -e` error, shellcheck does not flag it, and `bin/fm-spawn.sh` reported a successful spawn with no task record that way.
  A group's exit status is only its last command's, and errexit is suppressed inside a group used as the left operand of `||`, so `{ ... } > file || die` still hides a failure in the middle of the write when the group ends in a conditional command.
  Where a failure in the middle of the write has consequences, prepare the whole content in advance and write it with one checked simple command, as `bin/fm-spawn.sh` now does; checking the group's status with `|| die` is enough only when the group ends in the write itself.
