...
- Check the exit status of every `{ ... } > file` group explicitly (`|| die ...`), because bash 3.2, the stock macOS `/bin/bash`, does not treat a failed redirection of a compound command as a `set -e` error, and shellcheck does not flag it; `bin/fm-spawn.sh` reported a successful spawn with no task record that way.
