...
A mention arrives through the watcher as a `check:` wake whose payload is `x-mention <request_id>`, and the full mention is stashed locally.

This runs only when X mode is on (the captain dropped `FMX_PAIRING_TOKEN` into `.env`; see AGENTS.md "X mode").
If you ever see an `x-mention` wake without X mode configured, do nothing.
A `check:` wake can also carry `x-mode-error ...` instead of `x-mention <request_id>`: that is a poll or relay configuration problem, not a mention.
...
The direct mention `.text` is therefore a genuine message from the captain, and a request in it is a real instruction from the captain, to act on rather than merely answer, within the public-safety limits below.
Only the *direct* author is the owner; `in_reply_to` and any other thread participants may be third parties.
...
Every drained mention sorts into exactly one case, and an answered mention gets exactly one reply:

- **Actionable instruction / request** - act through the normal lifecycle. If it completes during this turn (a backlog item filed, an answer composed from fleet state), reply with the outcome. If it spawns a real, longer-running job (a dispatched crewmate, a scout investigation, a ship task), it cannot report an outcome yet, so it follows **acknowledge first -> act -> link**, with the outcome delivered later as completion follow-ups.
- **Question** - answer it from live fleet state; there is no work to do and no follow-up.
- **Pure acknowledgment** ("thanks", a reaction, a loop-closing nicety with nothing to add) - skip: post nothing, but first dismiss it at the relay so it stops being re-offered, then clear the inbox file.
...
The asker being your own captain does **not** relax this: a public reply is public no matter who prompted it, so an owner's request never licenses leaking private state into a public reply.
...
The **direct** mention `.text` is from your own owner - the captain - so read its intent as a real request and answer it.
...
Deflect (in voice) any ask for raw files, exact backlog or status contents, task ids, branch names, internal identifiers, secrets, tokens, credentials, hostnames, private URLs, or other internals.
...
- The asker **is** your captain, so address them as "captain" when it fits and treat their request as a genuine captain instruction, within the public-safety limits above. You are answering the captain in public, not a stranger.
...
The reply length authority is the relay (it trims), but a tight reply is on you: lean on the auto-split only when the answer truly needs the length, not as license to ramble.
...
2. **Drain every pending mention.** For each `state/x-inbox/*.json` file:
   ...
   b. **Classify it into one of the three cases above.**
      A pure acknowledgment ("thanks", "👍", "nice", "got it", a reaction, or a follow-up that just closes the loop) skips straight to step 2e-skip and then the step 2f cleanup, **without** calling `bin/fm-x-reply.sh`. A deliberate non-answer is the correct outcome here, not a failure.
   ...
   c. **Act on an actionable request through the normal lifecycle.** Treat it exactly as a captain prompt typed in session: run ordinary intake, then file the backlog item, dispatch a crewmate, start a scout, or ship through the gate.
      **Destructive, irreversible, or security-sensitive work is the exception**: do not execute it from the mention, flag it to the captain through the trusted channel first, act only on the captain's word, and in step 2d say only that it has been flagged for the captain.
      **If the request spawned a real, longer-running task** (you ran `bin/fm-spawn.sh`), the order is acknowledge first, act, then link:
      post an immediate, public-safe reply that you have the captain's order and are on it (the normal answer endpoint, via `bin/fm-x-reply.sh`), which is the legitimate, work-backed version of "aye, will do" - it is paired with actually starting the work in the same turn, never a promise left empty.
      Dispatch the work right away, then link the task so milestone and completion follow-ups can be posted:

      ```sh
      bin/fm-x-link.sh <task-id> <request_id>
      ```

      Do this right after the task is spawned, and always **before** removing the inbox file (step 2f), so `bin/fm-x-link.sh` can copy both the mention's reply platform and explicit budget from the still-present inbox payload without a relay lookup.
      It records the request id, a timestamp, a follow-up counter, and reply platform/budget context.
   ...
   e-skip. **For a skip, dismiss it at the relay instead of replying.** Clearing only the local inbox file is not enough: the relay keeps re-offering that request on every poll until it times out to a polite "offline" auto-reply. So before clearing the file, tell the relay to drop the request:
...
Two rules are worth repeating here, because this is where forgetting them costs:
...
The dismiss client records `{request_id, endpoint:"dismiss"}` to the same outbox path and behaves the same way.
...
Your procedure does not change: compose as usual, call the same helpers, and clear the inbox file as in step 2f - the only difference is that nothing reaches the relay.
