...
2. Run the script's `collect` command while the task's volatile records still exist.
   `bin/fm-spawn.sh` freezes the dispatch-time ship base to `base=<sha>` in `state/<id>.meta`, but only for a brief whose numbered branch step carries an explicit `origin/<ref>` argument, which the generated brief emits for the `--staging-autonomy` shape.
