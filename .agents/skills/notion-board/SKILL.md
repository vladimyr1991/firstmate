...
A `registry.complete` of false answers to `FM_SNAPSHOT_REGISTRY_RECORDS` and `FM_SNAPSHOT_SECONDMATES` the same way, and a `truncated` of true with homes reading `cross-home read deadline reached` answers to `FM_SNAPSHOT_CROSS_HOME_DEADLINE`; an `available: false` home is a real failure and never a retry.
