...
A standing PM with no per-cycle brief may instead use its own per-cycle self-computed live-link set as an equally valid `linked_cards` source: the cards that non-terminal tasks across all readable homes carry an active `notion_page=` link to (from `bin/fm-fleet-snapshot.sh --cross-home`).
A home whose links that snapshot did not fully enumerate makes that scan's sweep report unknown-not-orphaned for that home's cards rather than treating them as unlinked, and there are two such cases, not one: the home came back `available: false`, or its summary's `omitted[]` names the `endpoints` surface, which means its link list is bounded short of `counts.endpoints` and the missing tasks' cards would otherwise read as orphaned.
A snapshot whose `registry.available` is false is that case for the whole fleet: the roster itself could not be read, so no sibling home is enumerable and the sweep reports unknown-not-orphaned for every card it returns.
