...
A second mate whose scope turns out not to fit, or who declines, sends the card back, and it is sharpened and escalated as captain work on the existing path.
Firstmate opens a `captain`-kind decision holder in `data/backlog.md` whose note names the card and the second mate's reason, and names the sent-back card in its `resolved [key=dispatch]` line, or in the next PM brief or steer when the reply comes later.
The PM re-reads that card and, unless the captain moved it meanwhile, sets it to `На ревью`, so it leaves the eligible set and is never routed again while that holder stays open.
