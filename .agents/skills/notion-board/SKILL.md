...
A standing PM with no per-cycle brief may instead use its own per-cycle self-computed live-link set as an equally valid `linked_cards` source: the cards that non-terminal tasks across all readable homes carry an active `notion_page=` link to (from `bin/fm-fleet-snapshot.sh --cross-home`); a home the snapshot could not read makes that scan's sweep report unknown-not-orphaned for that home's cards rather than treating them as unlinked.
