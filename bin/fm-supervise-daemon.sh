...
# wedge_alarm_configured_channels, wedge_alarm_notify, and every notifier helper
# live in bin/fm-wedge-alarm-lib.sh (sourced above), shared with the
# outside-session watcher guard so both alarms have one implementation owner.
...
inject_msg() {  # <message> [state]
  ...
  if [ "$(fm_backend_text_bytes "$msg")" -gt "$(fm_backend_send_max_bytes)" ]; then
    local digest_file
    ...
    fm_operational_input_encode away-supervisor \
      "away-mode digest too long for the composer; read it in full at $digest_file" msg || return 1
