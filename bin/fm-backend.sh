...
# treats that as `tmux` (fm_backend_of_meta). That is a legacy allowance only:
# every new spawn writes `backend=`, tmux included, so an omitted field means
# the meta predates that change. See fm_backend_of_meta below for the rule.
#
