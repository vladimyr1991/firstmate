...
family_for_basename() {
  case "$1" in
    ...
    fm-supervision-events.test.sh|fm-turnend-guard.test.sh|fm-watcher-outside-guard.test.sh|fm-wake-daemon-lifecycle-e2e.test.sh|\
    ...
    fm-update.test.sh|fm-sprint-poll.test.sh)
      printf '%s\n' session-bootstrap
      ;;
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-ci-run-check.test.sh|fm-pr-check-security.test.sh|fm-pr-merge.test.sh|fm-review-diff.test.sh|\
    ...
    fm-fleet-snapshot-crosshome.test.sh)
      printf '%s\n' snapshot-bearings
      ;;
