...
if [ "$FM_SUP_NEEDED" = false ] || fm_watcher_healthy "$STATE" "$WATCH" "$GRACE" "$FM_HOME"; then rm -f "$EP"; printf 'watcher outside guard: healthy-or-idle\n'; exit 0; fi
[ -e "$EP" ] && { printf 'watcher outside guard: already-notified\n'; exit 0; }
now=$(date -u +%Y-%m-%dT%H:%M:%SZ); episode="$(date +%s)-${BASHPID:-$$}"; tmp=$(mktemp "$STATE/.watcher-outside-guard-episode.XXXXXX") || exit 1
...
if ! sed 's/^notified=0$/notified=1/' "$EP" > "$EP.tmp" || ! chmod 600 "$EP.tmp" || ! mv -f "$EP.tmp" "$EP"; then exit 1; fi
