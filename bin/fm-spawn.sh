...
#   is authorized, and owns the selection order and default applied without it).
...
ID=${POS[0]}
# The one genuine creation path, so this asks the creation question and carries
# every reservation; a site recognizing an id that already exists must not.
...
# for the --staging-autonomy shape. The scan is anchored to the NUMBERED branch
# step inside the brief's own `# Setup` section, because the author-supplied
