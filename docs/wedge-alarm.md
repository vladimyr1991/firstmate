# Wedge alarm and shared active alert

`bin/fm-wedge-alarm-lib.sh` is the single implementation of the backend-independent active alert: channel configuration, notifier timeout, command redaction, and the recorder seam.
Two callers source it.
The away-mode sub-supervisor (`bin/fm-supervise-daemon.sh`) buffers escalations and injects them into Firstmate's own pane; when injection cannot confirm a submit past `FM_MAX_DEFER_SECS`, `inject_wedge_alarm` raises a loud, rate-limited alarm so the stall never stays invisible.
The outside-session watcher guard (`bin/fm-watcher-outside-guard.sh`, see [`watcher-continuity.md`](watcher-continuity.md#active-limits-and-verification)) raises the same alert exactly once per unhealthy watcher episode.
...
This is deliberate because the alarm fires only after a genuine max-defer wedge, rate-limited to at most once per max-defer window, or once per outside-guard episode of lost watcher supervision.
...
On timeout or caller shutdown, the notifier process group is terminated and the next configured channel may run.
...
When the daemon is sourced as a library, that seam defaults to `discard`, so a test cannot accidentally post a real notification; the outside guard's tests set `discard` or a recorder explicitly.
...
`tests/fm-watcher-outside-guard.test.sh` covers the guard's once-per-episode alert through the same seam.
