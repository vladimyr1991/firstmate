...
A child that published nothing is terminated at the first deadline and recorded as `reason=confirmation-timeout`, while one that progressed and then stalled is terminated at the ceiling and recorded as `reason=confirmation-ceiling`.
Both print the unchanged `watcher: FAILED - no live watcher with a fresh beacon` and exit nonzero.
An adapter that waits for the arm to report readiness must keep its own timeout above that ceiling, or it retires a slow but working arm and relocates the same defect one layer up.
