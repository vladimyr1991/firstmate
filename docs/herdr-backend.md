...
Spawn-time projected cleanup (seeded default-tab prune, abort cleanup, reclaim) refuses to close the captain's active tab because a launch must never move focus; task retirement instead first moves focus to one exactly verified surviving tab, adopts it as the restoration authority, and still refuses when no surviving tab exists.
...
The current operational envelope starts with U+2063 and `FIRSTMATE_OP: `, except the launch-brief kind, which emits the same header fully visible with no leading U+2063.
...
tests/fm-backend-herdr-python-clients.test.sh
