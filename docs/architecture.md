...
A crew that `bin/fm-gate.sh task-state` reports as the live holder or a live waiter of the test-gate queue is in that queue's external wait: every stale escalation point in the watcher and the away-mode daemon absorbs it while the queue says so and asks again at its next threshold, and an unreadable queue leaves the ordinary escalation in place.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the operating checkout is tangled, if the home needs supervision under the shared predicate in [turnend-guard.md](turnend-guard.md#shared-predicate) without a healthy identity-matched watcher, or if queued wakes are waiting to be drained.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when that same predicate reports supervision need and no identity-matched watcher lock with a fresh beacon is live, direct Stop hooks block and passive turn-end hooks force one bounded follow-up.
...
Every new task writes `backend=`, `backend=tmux` included; for compatibility with metas written before that change, every reader still treats a missing `backend=` field as `tmux`.
...
A charter scaffolded with `fm-brief.sh --secondmate --standing-duty` is the sole exception, running one named standing duty on its own schedule, and only such a home is granted read-only visibility of the parent and the parent's registered siblings through `bin/fm-fleet-snapshot.sh --cross-home` and `--home-summary`; a secondmate's only cross-home write remains its parent status line, and the [`secondmate-provisioning` skill](../.agents/skills/secondmate-provisioning/SKILL.md) owns the duty boundary and that read grant.
...
`no-mistakes` tasks run the full validation pipeline, `direct-PR` tasks open PRs without that pipeline, and `local-only` tasks stay local by default until firstmate performs an approved fast-forward merge.
...
Standing staging-inclusive landing autonomy is therefore a `local-only` brief variant selected by [`bin/fm-brief.sh`](../bin/fm-brief.sh)'s `--staging-autonomy` flag rather than a fourth mode: the flag leaves that machine-readable line byte-identical so `bin/fm-spawn.sh` needs no coordinated change, and the script's header owns the generated contract.
...
Fleet state lives in each task's session-provider backend (tmux, herdr, cmux, zellij, or orca; [`configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) owns how one is selected and which is the default), no-mistakes run records, status event logs, local markdown under `data/` including `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, and persistent secondmate homes.
