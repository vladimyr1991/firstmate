...
It also covers the backfill and the ancestry-membership rule in live process trees: a daemon-parented session's legacy record upgraded onto the session rather than onto its daemon, a cleared daemon-parented session still reclaiming a record that names its own inner pid, and a live recorded owner outside this ancestry still refused with its record untouched.
...
It also covers the backfill's hook side: an owned legacy record upgraded in place, a backfilled home still arming after its recorded pid leaves the ancestry while staying alive, a legacy record with no resolvable session id left exactly as it was, and the upgrade refusing, without writing anything, both a record this session does not own and a lock that is not a regular file.
