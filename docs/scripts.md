...
| `fm-session-lock-lib.sh` | Shared session-lock record contract and harness identity (record parse, session id, ancestry walk, holder liveness) for fm-lock.sh, the Claude Stop auto-arm, and the turn-end guard |
