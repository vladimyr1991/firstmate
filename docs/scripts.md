...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`, optional `--staging-autonomy`), scout, secondmate-charter, and Herdr-lab briefs |
| `fm-gate.sh`             | Self-service machine-wide queue serializing FULL test-gate runs across task worktrees |
| `fm-herdr-lab.sh`        | Provision and guardedly operate an isolated, never-default Herdr lab session         |
| `fm-install-herdr.sh`    | Install CI's exact-version Herdr pin with official asset URL, SHA-256, and protocol checks |
| `fm-install-treehouse.sh`| Install CI's exact-version Treehouse pin for real-Herdr E2E that needs spawn worktrees |
| `fm-herdr-ci-cleanup.sh` | Snapshot and tear down only job-owned `fm-lab-*` sessions in the Herdr CI lane       |
| `fm-test-run.sh`         | Behavior-test runner: selection, portable lanes, proven-isolated `--jobs`, per-script process-group containment and timeout, coverage guard, timing/JSON |
| `fm-test-isolation-proof.sh` | Concurrent isolation proof and proven-isolated candidate set owner |
| `fm-ensure-agents-md.sh` | Ensure a project's real `AGENTS.md`, its `CLAUDE.md` symlink, and the canonical self-governance section |
| `fm-skill-trigger-check.sh` | Assert every tracked skill under `.agents/skills/` has a load trigger, declared as `user-invocable` or named in `AGENTS.md` |
| `fm-guard.sh`            | Warn on operating-checkout tangles, pending queued wakes, and stale watcher liveness |
| `fm-primary-scope-lib.sh` | Shared marker-or-plain-checkout primary-home predicate for tracked hooks             |
| `fm-session-lock-lib.sh` | Shared session-lock record contract and harness identity: record parse, session id, ancestry walk, holder liveness (docs/turnend-guard.md) |
...
| `fm-watcher-outside-guard.sh` | Outside-session watcher-loss detector for one explicit home: alert once per unhealthy episode, record bounded diagnostics, never restart (docs/watcher-continuity.md) |
| `fm-watcher-outside-guard-install.sh` | Install or remove the per-home macOS LaunchAgent that runs `fm-watcher-outside-guard.sh` without sudo |
...
| `fm-tangle-lib.sh`       | Operating-checkout resolution, default-branch resolution, and tangle classification |
| `fm-supervision-lib.sh`  | Shared supervision-need and fresh-watcher-beacon predicate (docs/turnend-guard.md "Shared predicate") |
...
| `fm-wedge-alarm-lib.sh`  | Single owner of the backend-independent active alert channels, notifier timeout, redaction, and `FM_WEDGE_ALARM_EXEC` seam (docs/wedge-alarm.md) |
...
| `fm-retro.sh`            | Collect countable signals from a landed task, record semantic attestation of lessons learned, and verify before teardown |
