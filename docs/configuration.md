...
FM_CAPTAIN_RE='(done|needs-decision|blocked|failed)( \[key=[^]]+\])?:|PR ready|checks green|ready in branch|merged'   # captain-relevant status regex; the optional [key=<slug>] token keeps keyed status lines matching, and nonterminal progress verbs remain excluded even when their prose matches
