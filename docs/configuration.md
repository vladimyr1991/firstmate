...
FM_OPENCODE_ARM_READY_TIMEOUT_MS=150000   # milliseconds the OpenCode primary watcher plugin waits for an arm attempt to report started, healthy, wake, or failure; must stay above the arm's confirmation ceiling (FM_ARM_CONFIRM_MAX)
FM_PI_ARM_READY_TIMEOUT_MS=150000   # milliseconds the Pi watcher extension waits for a successor arm to report started or attached before retiring it; must stay above the arm's confirmation ceiling (FM_ARM_CONFIRM_MAX)
