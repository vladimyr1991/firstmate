...
The per-backend delta is required only for the backend resolved from `FM_BACKEND`, then `config/backend`, then runtime auto-detection, then default `herdr` (with `tmux` as the announced last resort when herdr is not installed at all), so a home is never told to install a tool an inactive backend or feature would need.
