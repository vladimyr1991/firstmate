...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, per-project known-breakage records (`data/known-breakage/<repo>.md`, read by `bin/fm-brief.sh`), backlog, briefs, and scout reports.
...
New spawns choose the backend in this order: an explicit `--backend` flag that current authority for that exact task alone has authorized (a present captain instruction or the task's own accepted brief; never later-task precedent by analogy), then `FM_BACKEND`, then the first non-empty line of local gitignored `config/backend`, then runtime auto-detection from `$TMUX`, `HERDR_ENV=1`, or cmux runtime signals, then `herdr` as the fleet's base backend.
A set `config/backend` is authoritative rather than a default to route around: `fm-spawn.sh` refuses an explicit `--backend` naming anything else, because a task spawned outside the configured backend is invisible where the captain looks for it, so report an unusable configured backend instead of relocating the task, and move backends deliberately by editing `config/backend` or setting `FM_BACKEND` for one spawn.
`tmux` is the last resort only when `herdr` is not installed at all, and that substitution announces itself on stderr rather than happening silently.
This section is where that default is maintained, so other surfaces should cross-reference it rather than restate it and a correction lands here first.
...
Auto-detected herdr prints a stderr notice naming `config/backend` as the way to make that selection explicit, and auto-detected cmux prints one naming `config/backend` and `--backend tmux` as opt-outs; auto-detected tmux stays silent so an existing tmux-hosted home sees no change.
...
Task meta always records `backend=`, tmux included; readers still treat an absent `backend=` as `tmux` so meta files written before that change keep working.
...
## Active alert channels (config/wedge-alarm)

`config/wedge-alarm` (local, gitignored) and the `FM_WEDGE_ALARM_CHANNEL` override select the backend-independent active alert shared by the away-mode wedge alarm and the outside-session watcher guard.
[`wedge-alarm.md`](wedge-alarm.md) owns the directive reference, defaults, and test seam; [`examples/wedge-alarm`](examples/wedge-alarm) is the copyable config.
...
`FM_ROOT_OVERRIDE` overrides the firstmate repo root used by scripts, and takes precedence over every other input to the worktree-tangle guard's operating-checkout resolution, which otherwise follows `FM_HOME` and then the repo's main worktree (`bin/fm-tangle-lib.sh`).
...
The per-backend delta is required only for the backend actually resolved by the order in "Runtime backend" above, so a home is never told to install a tool an inactive backend or feature would need.
...
Bootstrap also reports a `TANGLE:` line when the resolved operating checkout is on a named non-default branch; follow the printed checkout remediation, which names that checkout, rather than treating it as an installable tool problem.
...
Independently of that lock, every bootstrap run emits `SECONDMATE_HARNESS_DRIFT:` for a registered secondmate whose recorded launch harness differs from the one a respawn would now resolve from `config/secondmate-harness`; the line is detect-only and never repins, relaunches, or rewrites the running secondmate.
...
FM_GATE_STALE_SECONDS=1500   # abandonment threshold of a gate-queue hold; see `bin/fm-gate.sh --help`, the one owner of what every FM_GATE_* threshold means
FM_GATE_MAX_HOLD_SECONDS=7200   # ceiling on a hold without a live heartbeat; see `bin/fm-gate.sh --help`
FM_GATE_RESOURCE_WAIT_SECONDS=3600   # how long `fm-gate.sh --wait` sits on a resource refusal before giving up; see `bin/fm-gate.sh --help`
FM_GATE_POLL_SECONDS=30      # seconds between `fm-gate.sh --wait` polls; see `bin/fm-gate.sh --help`
FM_GATE_HEARTBEAT_SECONDS=60   # seconds between the heartbeat touches of `fm-gate.sh run`; see `bin/fm-gate.sh --help`
FM_GATE_PARK_SECONDS=3600    # default deadline of `fm-gate.sh park`; see `bin/fm-gate.sh --help`
...
FM_CLAUDE_AUTOARM_EPOCH_FRESH=15   # seconds a recorded auto-arm outcome remains eligible for the current event epoch's recovery or failure decision, and the age past which an unmoved epoch counts as an auto-arm that is not participating
FM_CLAUDE_TURNEND_BLOCK_BUDGET=3   # consecutive --claude guard re-blocks before the one-time attended fail-open, which needs either a verified failure episode or an observably absent auto-arm; safely below Claude Code's 8-block override
FM_ARM_CONFIRM_TIMEOUT=45   # base seconds fm-watch-arm waits to confirm a fresh watcher; each NEW progress fact the child publishes (the singleton lock naming it, or a moved beacon) buys one further budget, so a slow start is not killed mid-confirmation; an empty, zero, or non-numeric value uses 45 rather than collapsing the budget
FM_ARM_CONFIRM_MAX=135   # hard ceiling in seconds on one arm's total confirmation wall clock, measured from fork; defaults to three times FM_ARM_CONFIRM_TIMEOUT and scales with it, clamps up to that base when set lower (which disables extension), uses that default for an empty, zero, or non-numeric value, and always terminates with the unchanged FAILED line
FM_ARM_ATTACH_POLL=0.5  # seconds between checks while fm-watch-arm is attached to an existing healthy watcher cycle
FM_OPENCODE_ARM_READY_TIMEOUT_MS=12000   # milliseconds the OpenCode primary watcher plugin waits for an arm attempt to report started, healthy, wake, or failure; default 35000 on Windows. Known gap: this does not cover the arm's silent worst case of FM_ARM_CONFIRM_MAX plus one FM_ARM_CONFIRM_TIMEOUT, and raising it alone multiplies against FM_WATCH_REARM_RETRY_LIMIT
FM_PI_ARM_READY_TIMEOUT_MS=12000   # milliseconds the Pi watcher extension waits for a successor arm to report started or attached before SIGTERMing it; default 35000 on Windows. Same known gap as the OpenCode timeout above
