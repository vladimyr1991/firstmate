...
FM_ARM_CONFIRM_TIMEOUT=45   # base seconds fm-watch-arm waits to confirm a fresh watcher; each NEW progress fact the child publishes (the singleton lock naming it, or a moved beacon) buys one further budget, so a slow start is not killed mid-confirmation
FM_ARM_CONFIRM_MAX=135   # hard ceiling in seconds on one arm's total confirmation wall clock, measured from fork; defaults to three times FM_ARM_CONFIRM_TIMEOUT and scales with it, clamps up to that base when set lower (which disables extension), and always terminates with the unchanged FAILED line
