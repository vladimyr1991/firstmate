...
- `fn+v`, and the grant round its first `start` asks for, are verified the same way; macOS ships several Fn-plus-letter shortcuts, listed for your machine under System Settings > Keyboard > Keyboard Shortcuts, and none of them uses `fn+v`, so nothing swallows the chord before the daemon sees it.
